...
.profile-btn { display: flex; align-items: center; gap: 8px; background: transparent; border: 1px solid var(--border); padding: 6px 16px; border-radius: 20px; font-weight: 500; cursor: pointer; color: var(--text-main); }
.relative { position: relative; }
.dropdown-overlay { position: fixed; inset: 0; z-index: 90; }
.dropdown { position: absolute; top: calc(100% + 10px); right: 0; width: 220px; background: white; border: 1px solid var(--border); border-radius: var(--radius-md); box-shadow: 0 10px 25px rgba(0,0,0,0.1); z-index: 100; display: flex; flex-direction: column;}
.dropdown-header { padding: 12px; background: #F9FAFB; font-size: 0.85rem; border-bottom: 1px solid var(--border); border-radius: 8px 8px 0 0;}
...
.text-danger { color: #EF4444; } .text-danger:hover { background: #FEF2F2; }
</style>